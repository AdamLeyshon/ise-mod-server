...
#[cfg(test)]
pub async fn response_body_to_bytes(response: HttpResponse) -> Vec<u8> {
    response.into_body().try_into_bytes().unwrap().to_vec()
}

#[cfg(test)]
pub fn create_protobuf_request() -> TestRequest {
    TestRequest::default()
...
impl<T: Message + Default + 'static> Future for ProtoBufMessage<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, task: &mut task::Context<'_>) -> Poll<Self::Output> {
        ...
        debug!(
            "Decoding Protobuf message, size limit: {} bytes",
            self.limit
        );
