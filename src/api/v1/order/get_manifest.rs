...
use crate::packets::order::{OrderManifestReply, OrderManifestRequest};
...
pub async fn action_get(
    _req: HttpRequest,
    bind: ClientBind,
    packet: ProtoBuf<OrderManifestRequest>,
) -> Result<HttpResponse> {
    if let Some(colony) = validate_ownership_and_fetch(None, Some(&packet.colony_id), &bind) {
        ...
        HttpResponse::Ok().protobuf(OrderManifestReply::from(order))
    } else {
